...
exports.loginVendor = exports.deleteVendor = exports.updateVendor = exports.getVendor = exports.getAllVendors = exports.createVendor = void 0;
...
async function createVendor(req, res, next) {
    ...
    try {
        const validationResult = utils_1.registerVendorsSchema.validate(req.body, utils_1.options);
        if (validationResult.error) {
            return res.status(400).json({
                Error: validationResult.error.details[0].message,
            });
        }
        const duplicateEmail = await vendorsModel_1.VendorSchema.findOne({
        ...
        const duplicatePhone = await vendorsModel_1.VendorSchema.findOne({
        ...
        const vendorDetails = await vendorsModel_1.VendorSchema.create(vendorData);
...
async function getAllVendors(req, res, next) {
    try {
        const vendorDetails = await vendorsModel_1.VendorSchema.findAll();
        res.status(201).json({
            status: "Success",
...
async function getVendor(req, res, next) {
    const { id } = req.params;
    try {
        const userDetails = await vendorsModel_1.VendorSchema.findOne({ where: { id } });
        res.status(201).json({
            status: "Success",
...
async function updateVendor(req, res, next) {
    try {
        const { id } = req.params;
        const vendorDetails = await vendorsModel_1.VendorSchema.findOne({ where: { id } });
        const { firstname, lastname, email, phone, bvn, local_govt, work_address, dob, state_of_origin, image_url, } = req.body;
        if (vendorDetails) {
            const updateUpdate = await vendorDetails.update({
                ...
                work_address: work_address || vendorDetails.getDataValue("work_address"),
                dob: dob || vendorDetails.getDataValue("dob"),
                state_of_origin: state_of_origin || vendorDetails.getDataValue("state_of_origin"),
...
async function loginVendor(req, res, next) {
    // const id = uuidv4();
    try {
        const validationResult = utils_1.loginVendorsSchema.validate(req.body, utils_1.options);
        if (validationResult.error) {
            return res.status(400).json({
                Error: validationResult.error.details[0].message,
            });
        }
        const User = (await vendorsModel_1.VendorSchema.findOne({
            where: { email: req.body.email },
        }));
        if (!User) {
